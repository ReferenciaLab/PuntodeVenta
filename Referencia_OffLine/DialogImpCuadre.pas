...
type
  TfrmDialogImpCuadre = class(TForm)
    ...
    qrCuadresTotal_PagoRD_Tres: TBCDField;
    qrCuadresDepo_Filtro2: TBCDField;
    qrCuadresDepo_Filtro2_Uno: TBCDField;
    qrCuadresDepo_Filtro2_Dos: TBCDField;
    qrCuadresCobros_Today_Tres: TBCDField;
    qrCuadresCobros_Anter_Tres: TBCDField;
    qrCuadresTotal_Filtro2: TBCDField;
    qrCuadresTotal_Filtro_Tres: TBCDField;
    qrCuadresTotal_Bruto_Tres: TBCDField;
    qrCuadresTotal_Cobertura_Tres: TBCDField;
    qrCuadresTotal_Descuento_Tres: TBCDField;
    qrCuadresTotal_Gastos_Tres: TBCDField;
    qrCuadresTotal_Pendiente_Tres: TBCDField;
    qrCuadresTotal_Credito_Tres: TBCDField;
    qrCuadresTotal_Contado_Tres: TBCDField;
    qrCuadresTotal_Venta_Tres: TBCDField;
    qrCuadresTotal_VtaRD_Tres: TBCDField;
    qrCuadresTotal_Normal_Tres: TBCDField;
    qrCuadresTotal_ContRD_Tres: TBCDField;
...
// Verifica Unificación de Filtro 1 y Filtro 2
      If dm.Verifica_Filtro_UnoDos(qrCuadres.FieldByName('SucursalId').AsString) then
         DM.Filtro_UnoDos :=1
      else
         DM.Filtro_UnoDos :=0;
...
////////////////////////////////////////////////////////////////////////////////
     ...
     Begin
        ...
        DM.qrCorteSucursal.Sql.Text :='SELECT CorteId, Usuario, Isnull(SUM(Valor),0) AS Valor,Isnull(SUM(ValorMST),0) AS ValorMST'+
        ...
        Texto := ' SELECT * FROM PTCobro C (nolock) Inner Join PTEntradaPaciente E (nolock) On C.refrecid = E.RecId '+
                 ' WHERE E.DataAreaId = ' + #39 + DM.CurEmpresa + #39 +
                 ' AND C.Fecha = '+#39+FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').AsDateTime)+#39+
        ...
        DM.qrCobro.Close;
        DM.qrCobro.SQL.Text := ' SELECT * FROM PTCobro C (nolock) Inner Join PTEntradaPaciente E (nolock) On C.refrecid = E.RecId '+
                               ' WHERE E.DataAreaId = ' + #39 + DM.CurEmpresa + #39 +
                               ' AND C.SinPrFilter = 0 '+
                               ' AND C.Fecha = '+#39+FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').AsDateTime)+#39+
                               ' AND C.SucursalId = :suc '+
...
//        DM.qrCobro.Parameters.ParamByName('fec').Value := FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').AsDateTime);
        ...
        DM.qrCobro.SQL.Text := DM.qrCobro.SQL.Text + ' AND C.Fecha = '+#39+FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').AsDateTime)+#39+
                               ' AND C.SucursalId = :suc '+
                               ' AND C.CuadreGlobal = :cdr '+
                               ' ORDER BY C.SucursalId, C.Fecha, C.UsuarioId, C.EntradaId, C.CobroId ';
        DM.qrCobro.Parameters.ParamByName('suc').Value := qrCuadres.FieldByName('SucursalId').AsString;
        ...
        //Imprimiendo el Deposito del Cuadre de Caja (FILTRO_DOS).
    ...
    //        DM.qrCobro.Parameters.ParamByName('fec').Value := FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').Value);
       ...
       Begin
         ...
         Texto := ' SELECT * FROM PTCobro C (nolock) Inner Join PTEntradaPaciente E (nolock) '+
                  ' ON C.refrecid = E.RecId '+
                  ' WHERE E.DataAreaId = ' + #39 + DM.CurEmpresa + #39 +
                  ' AND C.Fecha = '+#39+FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').AsDateTime)+#39+
                  ' AND C.SucursalId = :suc '+
...
//         DM.qrCobro.Parameters.ParamByName('fec').Value := FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').Value);
         ...
         //Imprimiendo el Cuadre de Caja (FILTRO DOS).
        If DM.Filtro_UnoDos = 0 then
        begin
           DM.qrCobro.Close;
           DM.qrCobro.SQL.Text := ' SELECT * FROM PTCobro C (nolock) Inner Join PTEntradaPaciente E (nolock)'+
...
////////////////////////////////////////////////////////////////////////////////
     ...
     DM.qrCobro.Parameters.ParamByName('suc').Value := qrCuadres.FieldByName('SucursalId').AsString;
//     DM.qrCobro.Parameters.ParamByName('fec').Value := FormatDateTime('dd/mm/yyyy', qrCuadres.FieldByName('Fecha').Value);
     ...
     If (DM.Cobro_Var = 'Usuario') Then
       Texto := ' SELECT * FROM PTEntradaPaciente (nolock) '+
                ' WHERE Fecha = '+#39+FormatDateTime('yyyymmdd', qrCuadres.FieldByName('Fecha').AsDateTime)+#39+
