...
type
  TfrmDatosClientesTarjeta = class(TfrmDatosModule)
...
var
  frmDatosClientesTarjeta: TfrmDatosClientesTarjeta;

implementation

uses DataModule, Main, DatosTarjeta, UInterfaseAs400;

{$R *.dfm}


procedure TfrmDatosClientesTarjeta.DoPost;
Var
  qfind       : TAdoQuery;
begin
...
procedure TfrmDatosClientesTarjeta.EdCedulaExit(Sender: TObject);
Var
  qfind       : TAdoQuery;
  Direccion   : String;
begin
  ...
  If ((qrClientes.State = dsInsert) Or (qrClientes.State = dsEdit)) and
     (Length(trim(EdCedula.Text))=14)  Then
    Begin
...
procedure TfrmDatosClientesTarjeta.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmDatosClientesTarjeta.FormShow(Sender: TObject);
Var
  hwndText : THandle;
begin
  inherited;
...
procedure TfrmDatosClientesTarjeta.qrClientesBeforeDelete(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmDatosClientesTarjeta.qrClientesBeforeEdit(DataSet: TDataSet);
Var
  Limpio : String;
  qfind : TAdoQuery;
begin
        ...
        Begin
          ...
          If (qrClientesGrupoCliente.Value = DM.qrParametroGrupoAcc.Value) Or
            ...
            Begin
             ...
             qfind.SQL.Text := ' SELECT NOMBRES,APELLIDO1,ISNULL(APELLIDO2,'+#39+#39+') AS APELLIDO2, FECHA_NAC, SEXO, COD_SANGRE  FROM '+Db1Servidor+'..CEDULADOS '+
...
procedure TfrmDatosClientesTarjeta.qrClientesBeforeInsert(DataSet: TDataSet);
begin
  inherited;
  lcDatosTipos.Enabled := False;
  lcDatosGrupos.Enabled := False;
...
procedure TfrmDatosClientesTarjeta.qrClientesBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  buscar := False;
end;

procedure TfrmDatosClientesTarjeta.qrClientesBeforePost(DataSet: TDataSet);
Var
 _valor  : Integer;
 qsecdoc : TADOQuery;
begin
...
procedure TfrmDatosClientesTarjeta.qrClientesFECHANACIMIENTOValidate(Sender: TField);
begin
  inherited;
  If (qrClientes.State = dsInsert) Or (qrClientes.State = dsEdit) Then
    Begin
...
procedure TfrmDatosClientesTarjeta.qrClientesGRUPOCLIENTEChange(Sender: TField);
begin
  inherited;
  Refrescar;
end;
...
procedure TfrmDatosClientesTarjeta.qrClientesIDENTIFICACIONValidate(Sender: TField);
Var
  qfind : TAdoQuery;
  b     : boolean;
begin
      ...
      Begin
         ...
         begin
             If (EtMensajeDlg(' El número de cédula debe ser único.  El cliente '+ qfind.FieldByName('Nombre').AsString +') lo tiene registrado. '+ #13 +
              ...
              begin
                 frmMain.CustTarjeta := qfind.Fieldbyname('ClienteID').AsString;
                 qrClientes.BookMark;
                 qrClientes.Cancel;
                 qrClientes.Edit;
                 qrClientes.GetBookMark;
...
procedure TfrmDatosClientesTarjeta.qrClientesNewRecord(DataSet: TDataSet);
Var RecId: Int64;
begin
  inherited;
...
procedure TfrmDatosClientesTarjeta.qrClientesORIGENChange(Sender: TField);
begin
  inherited;
  Refrescar;
end;
...
procedure TfrmDatosClientesTarjeta.Refrescar;
Var
  Limpio : String;
  qfind : TAdoQuery;
begin
...
procedure TfrmDatosClientesTarjeta.qrClientesAfterOpen(DataSet: TDataSet);
begin
  inherited;
  buscar := True;
end;

procedure TfrmDatosClientesTarjeta.qrClientesAfterPost(DataSet: TDataSet);
begin
...
procedure TfrmDatosClientesTarjeta.qrClientesAfterScroll(DataSet: TDataSet);
begin
  inherited;
  Refrescar;
end;
...
procedure TfrmDatosClientesTarjeta.Procesar_Usuario;
Var
  Valor : Boolean;
...
Function TfrmDatosClientesTarjeta.Crear_Clave(cedularnc : String) : Integer;
Var
  Numero  : Double;
  Valor   : Int64;
//  Valor   : Integer;
...
//Creando el Usuario de Internet.
Function TfrmDatosClientesTarjeta.Crear_Usuario(nombre : String) : String;
Var
  Texto : String;
...
Function TfrmDatosClientesTarjeta.Valida_Usuario(usuario : String) : Boolean;
Var
 qfind  : TAdoQuery;
begin
...
procedure TfrmDatosClientesTarjeta.Insertar_Usuario(usuario: string; clave: string; name: string; cedula: string;
...
//Validando la Existencia del Usuario Creado.
Function TfrmDatosClientesTarjeta.Buscar_User_Internet(Cliente : String) : Boolean;
Var
 qfind  : TAdoQuery;
begin
  qfind := DM.NewQuery;
...
procedure TfrmDatosClientesTarjeta.Nuevo_Registro;
Var
  hwndText : THandle;
begin
