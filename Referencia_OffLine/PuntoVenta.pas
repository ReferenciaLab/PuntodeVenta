...
//  frmMain.LanzaVentana(-8021); //Confirmando la Cobertura Tardia.
  ...
  Begin
          ...
          (qrEntradaPacienteOrigen.Value <> DM.qrParametroGrupoSeg.Value) then
  Begin
          EtMensajeDlg('Tipo de Cliente NO permite confirmación de Cobertura.  Verifique.', etAviso, [etOk],0, dm.Imagenes.Items[4].Bitmap,true)
  end
  else
...
//          CUsuario        := GetPalicSoap(false,Dm.vURLWSDL).fValidaPassword('REFERENCIA','REFERENCIA');
          ...
          begin
            ...
            Except
                Raise exception.CreateFmt('Verifique No. de Afiliado ó Contrato.', []);
            end;
...
begin
  inherited;
   if (qrEntradaPacienteCoberturaConfirmada.Value = 1) Or
     ...
     EtMensajeDlg('La Cobertura Ha Sido Confirmada ó Rechazada.  El Cliente NO Puede Ser Cambiado.', etError, [etOk],0, dm.Imagenes.Items[4].Bitmap,true)
   else
   begin
...
begin
   ...
   Begin
      ...
      begin
          strDocumento := InputBox('Número de Documento :', 'No. Documento:',qconsultaDoc.FieldByName('Identificacion').AsString);
          Clipboard.AsText := qconsultaDoc.FieldByName('Identificacion').AsString;
      end
      else
          ShowMessage('Número de Documento NO EXISTE.');
...
begin
 ...
 begin
      if DM.Mensaje('Fecha indicación médica en blanco.  Desea digitar fecha indicación ?',mb_yesno) = id_yes then
      begin
          exit;
      end;
 end;
 ...
 begin
    ...
    begin
      if DM.Mensaje('La Paciente está embarazada?',mb_yesno) = id_yes then
      begin
 ...
 begin
   ...
   begin
       ...
       begin
          ...
          If dm.CurRol <> 7  then
          Begin
              Id_Visita := InputBox('Inserte número de ID VISITA :', 'ID Visita:','');
              if Id_Visita <> EmptyStr then
              begin
                  If dm.Verifica_Id_Visita(Id_Visita,qrEntradaPacienteMuestrano.Value) = False then
                  begin
                     If (EtMensajeDlg('ID VISITA digitado no válido o ya asignado.  Desea registrar la visita ?',etConfirmacion,[etSi,etNo],1,dm.Imagenes.Items[5].Bitmap,true) = mryes) then
                     begin
                        If qrEntradaPaciente.State in [dsInsert] Then
                        Begin
                  ...
                  Raise exception.CreateFmt('Debe insertar número de ID VISITA válido.  Verifique.', []);
          end;
       end;
...
{   If (qrEntradaPacienteCoberturaSeguro.Value > 0) And
   ...
   Begin
     if MessageDlg('Número Aprobación está en blanco.  Desea Confirmar Cobertura?', mtConfirmation, [mbYes,mbNo], 0) = mrYes then
     begin
      ConfirmarCobertura1Click(Sender);
     end;
   end;
...
//   Si es Nuevo o está en Hold entonces entra
     ...
     begin
        If (from_hold = True) Or (copy(qrEntradaPacienteEntradaID.value, 1 ,3) = 'HOL') Then
          If MessageDlg('Esta Cotización se convertirá en Factura.  Desea continuar?', mtConfirmation, [mbYes,mbNo], 0) = mrNo  then Exit;
...
//        Mirar_Valor_CAB;
   ...
   //Generando las Acciones del Boton COBRAR Original.
     ...
     begin
          if DM.Mensaje('Factura a Crédito.  Está seguro de realizar el pago ?',mb_yesno) = id_yes then
          begin
     ...
     begin
      if DM.Mensaje('Desea Poner en Hold?',mb_yesno) = id_yes then
       begin
          ...
          if DM.Mensaje('Desea Imprimir Cotización?',mb_yesno) = id_yes then
          begin
...
begin
  ...
  If (qrEntradaPacienteCoberturaConfirmada.Value = 1) And
     (qrEntradaPacienteOrigen.Value = DM.qrParametroGrupoSeg.Value) then
  Begin
          EtMensajeDlg('Prueba NO puede eliminarse Ya está Confirmada.  Verifique.', etError, [etOk],0, dm.Imagenes.Items[6].Bitmap,true);
          Abort;
  end;
  If not ((Trim(qrEntradaPacienteEntradaid.value) ='') Or (Copy(qrEntradaPacienteEntradaid.value,1,3)='HOL')) then
  begin
...
//                     If (Not(InterfaseAS400.User_Internet_AS400 <> EmptyStr) And (InterfaseAS400.Clave_Internet_AS400 <> EmptyStr)) Then
//                     Begin
                         If Not((Trim(InterfaseAS400.User_Internet_AS400)=EmptyStr) Or   (Trim(InterfaseAS400.Clave_Internet_AS400)=EmptyStr) Or
...
begin
  ...
  begin
     Raise exception.CreateFmt('Número de Afiliado No puede estar en blanco.  Verifique.', []);
     if cxDBTextEdit16.enabled then  cxDBTextEdit16.SetFocus;
  end
  else if (Trim(qrEntradaPacienteCOD_DIAG.Value) = EmptyStr) Then
  begin
     Raise exception.CreateFmt('Código de diagnóstico No puede estar en blanco.  Verifique.', []);
     if EDDIAGN.enabled then  EDDIAGN.SetFocus;
  end
  else If Trim(qrEntradaPacienteNOMBREDOCTOR.Text) = EmptyStr Then
  begin
