...
begin
  ...
  begin
     If (EtMensajeDlg('Está seguro de eliminar pedido del Cliente(s) ',etConfirmacion,[etSi,etNo],1,dm.Imagenes.Items[5].Bitmap,true) = mryes) then
     begin
        qrPedido_Head.First;
        While Not qrPedido_Head.Eof do
        begin
...
{       Fecha_Proceso := dtFechaDesde.Date;
       ...
       begin
            ...
            begin
               ...
               begin
                    spMensaje.Visible := False;
                     Raise exception.CreateFmt( ' Verifique Tasa del Dia en Sap '+FormatDateTime('dd/mm/yyyy',qrPedido_HeadFecha_Hasta.AsDateTime)+' no está registrada.  Verifique.',[]);
               end;
