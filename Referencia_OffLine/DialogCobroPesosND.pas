...
begin
  ...
  begin
    ...
    begin
       Raise exception.CreateFmt('Debe registrar número de aprobación de la Tarjeta de Crédito.  Verifique.', []);
...
{    If Trim(edNumeroTarC.Text)=EmptyStr Then
  ...
  begin
    ...
    begin
       Raise exception.CreateFmt('Debe registrar número de aprobación de la Tarjeta de Débito.  Verifique.', []);
    ...
    end;
{
  ...
  If (frmMain.frmTmpND.tpago > 0) Then
  begin
    If Trim(edTransaccion.Text)=EmptyStr Then
    begin
       Raise exception.CreateFmt('Debe registrar número de transacción de T-Pago.  Verifique.', []);
...
//                else
                frmMain.frmTmpND.mdPagoFecha_Vencimiento.Value :=StrToDate('01/01/1900');
                frmMain.frmTmpND.mdPagoNumero_Aprobacion.Value := Trim(edAprobacionTarC.EditValue);
                frmMain.frmTmpND.mdPago.Post;
             end;
...
//                else
                frmMain.frmTmpND.mdPagoFecha_Vencimiento.Value :=StrToDate('01/01/1900');
                frmMain.frmTmpND.mdPagoNumero_Aprobacion.Value := Trim(edAprobacionTarD.EditValue);
                frmMain.frmTmpND.mdPago.Post;
             end;
             If (frmMain.frmTmpND.mdPagoFormaPagoID.Value = 'TP') Then
             begin
                frmMain.frmTmpND.mdPago.Edit;
                frmMain.frmTmpND.mdPagoNumero_Aprobacion.Value := Trim(edTransaccion.EditValue);
                frmMain.frmTmpND.mdPago.Post;
             end;
  ...
  if (frmMain.frmTmpND.qrEntradaPacienteDESCUENTO.Value > 0) and
     ...
     (frmMain.frmTmpND.qrEntradaPacienteOrigen.value = DM.qrParametroGrupoPriv.Value) And
     (Trim(frmMain.frmTmpND.qrEntradaPacienteDescuentoCard.value) = EmptyStr) And
     (Trim(frmMain.frmTmpND.qrEntradaPacienteDescuentoTexto.value) = EmptyStr) And
     (Trim(frmMain.frmTmpND.qrEntradaPacienteDescuentoPlanID.value) = EmptyStr) Then
  begin
...
procedure TfrmDialogCobroPesosND.btClearTPClick(Sender: TObject);
begin
