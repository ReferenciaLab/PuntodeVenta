...
begin
 ...
 FreeAndNil(qfindSucursal);
// Incluir Total Credito = Monto a Cubrir por el Seguro + Entradas a Crédito
...
begin
  ...
  EAlert.HeaderText := 'Error de Excepción';
...
//Función Para Eliminar Una Caracter Especifico de Un Texto Dado.
...
begin
  LicenseError('Error de inicialización',True);
end;
...
begin
  LicenseError('El ejecutable ha sido alterado, verifique por virus ó reinstale',True);
end;
...
begin
  LicenseError('Código de  licencia invalido, favor de verificar.',True);
end;
...
begin
  LicenseError('Cantidad máxima de usuarios excedida.',True);
end;


procedure TDM.IceLicenseNetworkReject(Sender: TObject);
begin
  LicenseError('Esta PC no esta autorizada en la red para usar la aplicación.',True);
end;
...
begin
     ...
     begin
        Close;
        Text := ' Update PtEntradaPaciente set Nota = '+#39+'Nota de Crédito ('+TipoNC+')'+#13+
                ' Monto = '+CurrToStr(Monto)+#13+'Empleado = '+DescSalida+#13+'Usuario Creación = '+dm.CurUser+#39+
                ' Where Recid='+RecID;
        ExecSQL;
     end;
     FreeAndNil(qinsert);
  ...
  begin
    ...
    //        WriteLn(#27+'G'+#27+Char(14)) ;
            ...
            begin
              ...
              WriteLn(Justifica('Fecha de Emisión: '+FormatDateTime('dd/mm/yy',qSerial.FieldByName('Fecha_Emision').Value),30,' ',tjLeft));
              WriteLn(Justifica('Fecha de Revisión: '+FormatDateTime('dd/mm/yy',qSerial.FieldByName('Fecha_Revision').Value),30,' ',tjLeft));
              WriteLn(#10);
              WriteLn(#10);
              WriteLn(#10);
            end;
...
begin
  ...
  begin
    ...
    //        WriteLn(Justifica('Autorizacion :'+Copy(Trim(fieldbyname('Autorizacion').AsString),1,25),PP,' ',tjLeft));
            ...
            WriteLn(RellenaLinea(13,' ')+Justifica('Devolución     $ ',17,' ',tjLeft)+Justifica(FormatFloat('##,##0.00', fieldbyname('TotalCobrado').AsFloat),10,' ',tjRight));
            WriteLn(#10);
            WriteLn(#10);
            WriteLn(RellenaLinea(PP,'-'));
            WriteLn(Justifica('Firma del Paciente',PP,' ',tjCenter));
